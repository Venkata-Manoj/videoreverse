...
from src.blueprint_prompt import BLUEPRINT_SYSTEM_PROMPT, BLUEPRINT_SCHEMA
...
def _build_frame_context(timeline_frames):
...
def _detect_motion_transitions(timeline_frames):
...
def _group_frames_by_motion(timeline_frames):
...
def _build_shot_boundary_hints(scene_changes):
...
def _extract_motion_transitions(timeline_frames):
...
async def build_blueprint(video_path, step1_data, options=None):
    if options is None:
        options = {}
    ...
    try:
        ...
        audio_info = f'Audio mood: {audio_mood["mood"]}' if audio_mood and audio_mood.get('mood') else f'Audio: {"Yes" if audio.get("has_audio") else "No"}'
...
Use these as additional hints for shot boundary detection."""
        ...
        if motion_transitions:
            print(f'   → Motion transitions: {len(motion_transitions.split(", ")) if isinstance(motion_transitions, str) else 0} likely cut points from frame analysis', flush=True)
        ...
        blueprint['_metadata'] = {
            ...
            'shots_with_frame_traceability': len([
                s for s in blueprint['chronological_shots']
        ...
        blueprint['_metadata']['analysis_timestamp'] = datetime.now(timezone.utc).isoformat()

        print(f'✅ Frame-aware blueprint generated:', flush=True)
        ...
        if scene_changes:
            ...
            if variance > 2:
                print(f'   ⚠️ Shot count mismatch: expected ~{expected_shots} shots based on {len(scene_changes)} detected cut points, got {shot_count}', flush=True)
