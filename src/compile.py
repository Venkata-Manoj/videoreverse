...
def _resolve_aspect_ratio(width, height):
...
def _fill_template(template, vars_dict):
    result = template
...
def _apply_enhancement_rules(prompt, model_config):
    ...
    if keyword_phrases:
        ...
        if relevant_keywords:
            ...
            if (guidelines.get('emphasize_comma_format') or guidelines.get('emphasize_physics') or
                    guidelines.get('emphasize_environment') or guidelines.get('emphasize_motion') or
                    guidelines.get('emphasize_atmosphere') or guidelines.get('emphasize_temporal_flow') or
                    guidelines.get('emphasize_style') or guidelines.get('emphasize_animation') or
                    guidelines.get('emphasize_realism') or guidelines.get('emphasize_quality')):
...
def compile_prompts(blueprint, video_metadata, filter_models=None):
        ...
        for shot in shots:
            ...
            vars_dict = {
                ...
                'aspect_ratio': (
                    ...
                    else (model_config.get('aspect_ratio_support', ['16:9'])[0] if model_config.get('aspect_ratio_support') else '16:9')
                ),
            }
