...
def _get_video_duration(video_path):
    try:
        ...
        raise RuntimeError('ffprobe not found. Install ffmpeg to use smart sampling.')
    except subprocess.CalledProcessError as e:
        raise RuntimeError(f'ffprobe failed: {e.stderr}')


def _clip_first_n(video_path, duration_seconds):
    ...
    try:
        subprocess.run(
            ['ffmpeg', '-i', video_path, '-t', str(duration_seconds), '-c', 'copy', '-avoid_negative_ts', 'make_zero', clipped_path],
            capture_output=True, text=True, check=True
        )
...
def _extract_highlights(video_path, target_duration=30):
    ...
    for i in range(total_segments):
        ...
        try:
            result = subprocess.run(
                ['ffmpeg', '-i', video_path, '-ss', str(start), '-t', str(actual_duration),
    ...
    for i, seg in enumerate(top_segments):
        filter_parts.append(
            f'[0:v]trim=start={seg["start"]}:duration={seg["duration"]},setpts=PTS-STARTPTS[v{i}]'
        )
    ...
    try:
        subprocess.run(
            ['ffmpeg', '-i', video_path, '-filter_complex', filter_complex,
             '-map', '[outv]', '-c:v', 'libx264', '-preset', 'fast', '-crf', '23', '-an', '-y', highlights_path],
            capture_output=True, text=True, check=True
        )
    except subprocess.CalledProcessError:
...
def sample_video(video_path, options=None):
    if options is None:
        options = {}

    sample_mode = options.get('sample_mode', 'full')
    max_duration = options.get('max_duration', None)
    ...
    if sample_mode == 'first-n':
        ...
        if clip_duration >= full_duration:
            print(f'   → Requested {clip_duration}s ≥ video duration {full_duration:.1f}s — using full video', flush=True)
            return {
                'path': video_path,
                'temp_dir': None,
                'mode': 'full',
...
def cleanup_sample(sample_result):
