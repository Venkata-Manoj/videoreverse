...
def _get_levenshtein_similarity(str1, str2):
    len1 = len(str1)
    len2 = len(str2)
...
def compare_prompts(old_data, new_data):
    results = {
        'timestamp': None,
        'models': {},
    }
    ...
    for model in all_models:
        ...
        avg_similarity = (
            sum(c['similarity'] for c in prompt_changes) / len(prompt_changes)
            if prompt_changes else 100
        )
...
def save_comparison(baseline_path, new_path, output_path=None):
    try:
...
def print_comparison(compare_result):
